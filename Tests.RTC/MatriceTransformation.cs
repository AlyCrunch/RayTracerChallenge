...
namespace Tests.RTC
{
    public class MatriceTransformation
    {
        ...
        public void MultiplyingTranslationMatrix()
        {
            ...
            var p = RTF.PointType<decimal>.Point(-3, 4, 5);
            var expected = RTF.PointType<decimal>.Point(2, 1, 7);
        ...
        public void MultiplyingInvertedTranslationMatrix()
        {
            ...
            var p = RTF.PointType<decimal>.Point(-3, 4, 5);
            var expected = RTF.PointType<decimal>.Point(-8, 7, 3);
        ...
        {
            var transform = RTH.Transformations.Translation(5, -3, 2);
            var v = RTF.PointType<decimal>.Vector(-3, 4, 5);
        ...
        public void ScalingMatrixPoint()
        {
            ...
            var p = RTF.PointType<decimal>.Point(-4, 6, 8);
            var expected = RTF.PointType<decimal>.Point(-8, 18, 32);
        ...
        {
            var transform = RTH.Transformations.Scaling(2, 3, 4);
            var p = RTF.PointType<decimal>.Vector(-4, 6, 8);
            var expected = RTF.PointType<decimal>.Vector(-8, 18, 32);
        ...
        public void InverseScalingMatrix()
        {
            ...
            var expected = RTF.PointType<decimal>.Vector(-2, 2, 2);
        ...
        public void ReflexionScalingNegativeValue()
        {
            ...
            var p = RTF.PointType<decimal>.Point(2, 3, 4);

            var expected = RTF.PointType<decimal>.Point(-2, 3, 4);
        ...
        {
            var p = RTF.PointType<decimal>.Point(0, 1, 0);

            var halfQuarter = RTH.Transformations.RotationX(Math.PI / 4);
            var expected1 = RTF.PointType<decimal>.Point(0, (decimal)Math.Sqrt(2) / 2, (decimal)Math.Sqrt(2) / 2);
            Assert.Equal(expected1, halfQuarter * p);

            var fullQuarter = RTH.Transformations.RotationX(Math.PI / 2);
            var expected2 = RTF.PointType<decimal>.Point(0,0,1);
